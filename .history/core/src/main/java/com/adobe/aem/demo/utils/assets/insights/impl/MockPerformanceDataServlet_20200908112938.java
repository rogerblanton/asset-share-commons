...
import com.adobe.aem.demo.utils.assets.insights.impl.MockAssetUsageRecord.ASSET_USAGE_TYPE;
...
public class MockPerformanceDataServlet extends SlingSafeMethodsServlet {
    ...
    protected void doGet(SlingHttpServletRequest request,
        ...
        try {
            ...
            Double usageFactor = getUsageMultiplier(asset);
            ...
            log.debug("usageFactor [ {} ]", usageFactor);

            log.debug("Offset [ {} ]", offset);
            
            JSONObject json = null;
            if ("day".equals(granularity)) {
                json = this.generateDays(fromDate, toDate, seed, offset, usageFactor);
            } else if ("week".equals(granularity)) {
                json = this.generateWeeks(fromDate, toDate, seed, offset, usageFactor);
            } else if ("month".equals(granularity)) {
                json = this.generateMonths(fromDate, toDate, seed, offset, usageFactor);
            }
    ...
    private JSONObject generateDays(Date fromDate, Date toDate, int seed, int offset, double usageFactor) throws JSONException {
        ...
        for (Date fromIter = fromDate; !fromIter.after(toDate); fromIter = DateUtils.addDays(fromIter, 1)) {
            ...
            metrics.add(String.valueOf(new Double(usageFactor * (base + plus)).intValue()));
        }
    ...
    private JSONObject generateWeeks(Date fromDate, Date toDate, int seed, int offset, double usageFactor) throws JSONException {
        JSONObject json = new JSONObject();
        ...
        for (Date fromIter = fromDate; !fromIter.after(toDate); fromIter = DateUtils.addWeeks(fromIter, 1)) {
            ...
            metrics.add(String.valueOf(new Double(usageFactor * 7 * (base + plus)).intValue()));
        }
    ...
    private JSONObject generateMonths(Date fromDate, Date toDate, int seed, int offset, double usageFactor) throws JSONException {
        JSONObject json = new JSONObject();
        ...
        for (Date fromIter = fromDate; !fromIter.after(toDate); fromIter = DateUtils.addMonths(fromIter, 1)) {
            ...
            metrics.add(String.valueOf(new Double(usageFactor * 30 * (base + plus)).intValue()));
        }
    ...
    private Double getUsageMultiplier(Resource resource) {
        
        Resource usages = resource.getChild("jcr:content/usages");

        if (usages == null) {
            return 0D;
        }

        StreamSupport.stream(usages.getChildren().spliterator(), false).map(r -> r.getValueMap()).map(p -> p.get("count", 0D)).mapToDouble(num -> new Double(num)).sum();

        resource.

        Double MAX_USAGE_FACTOR = 25D;

        List<String> usageTypes = Arrays.asList(
            ASSET_USAGE_TYPE.AEM.name(), 
            ASSET_USAGE_TYPE.ASSET.name(),
            ASSET_USAGE_TYPE.CAMPAIGN.name(),
            ASSET_USAGE_TYPE.MEDIA_OPTIMIZER.name(),
            ASSET_USAGE_TYPE.SOCIAL.name(), 
            ASSET_USAGE_TYPE.TARGET.name());
            List<AssetUsageTracker.AggregateUsageInfo> usage = assetUsageTracker.getUsageStats(asset, usageTypes);

            Double total = usage.stream().mapToDouble(u -> u.usageCount).sum();
            if (total > MAX_USAGE_FACTOR) {
                total = MAX_USAGE_FACTOR;
            }
            
            return new Double(total) / MAX_USAGE_FACTOR;
    }
