...
public class MockPerformanceDataServlet extends SlingSafeMethodsServlet {
    ...
    protected void doGet(SlingHttpServletRequest request,
        ...
        try {
            ...
            Double usageFactor = getUsageMultiplier(resource);

            if ("impressions".equals(metric)) {
                offset = randomInRange(BigInteger.valueOf(seed), cfg.minBaseline(), cfg.maxBaseline());
    ...
    private int randomInRange(BigInteger hash, int min, int max) {
        int rand = hash.mod(BigInteger.valueOf(max)).intValue();
    ...
    private Double getUsageMultiplier(Resource resource) {
        ...
        Double total = StreamSupport.stream(usages.getChildren().spliterator(), false)
            .map(r -> r.getValueMap())
            .map(p -> p.get("count", 0D))
            .mapToDouble(num -> new Double(num))
            .sum();
